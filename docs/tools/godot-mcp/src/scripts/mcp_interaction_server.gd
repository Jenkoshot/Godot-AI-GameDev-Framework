...
func _cmd_get_node_info(params: Dictionary) -> void:
	...
	for prop: Variant in node.get_property_list():
	...
	var signals: Array = []
	for sig: Variant in node.get_signal_list():
		var sig_dict: Dictionary = sig
		signals.append(sig_dict.get("name", ""))

	var methods: Array = []
	for m: Variant in node.get_method_list():
...
func _cmd_play_animation(params: Dictionary) -> void:
	...
	match action:
		...
		"get_list":
			var anims: Array = []
			for anim_name: Variant in anim_player.get_animation_list():
...
func _cmd_get_audio() -> void:
	var buses: Array = []
	for i: Variant in AudioServer.bus_count:
		buses.append({
...
func _cmd_navigate_path(params: Dictionary) -> void:
	...
	if is_3d:
		...
		for i: Variant in range(1, path.size()):
...
func _cmd_manage_group(params: Dictionary) -> void:
	...
	match action:
		...
		"get_groups":
			var groups: Array = []
			for g: Variant in node.get_groups():
				groups.append(str(g))
...
func _cmd_create_animation(params: Dictionary) -> void:
	...
	for track_data: Variant in tracks:
...
func _deserialize_node(node: Node, data: Dictionary) -> int:
	...
	for child_data: Variant in children_data:
		...
		var child: Node = null
		for c: Variant in node.get_children():
...
func _cmd_bone_pose(params: Dictionary) -> void:
	...
	match action:
		"list":
			var bones: Array = []
			for i: Variant in skel.get_bone_count():
...
func _cmd_ui_theme(params: Dictionary) -> void:
	...
	for name: Variant in font_sizes:
...
func _clear_debug_draw() -> void:
	for entry: Variant in _debug_meshes:
...
func _cmd_list_signals(params: Dictionary) -> void:
	...
	for sig: Variant in node.get_signal_list():
		...
		for conn: Variant in node.get_signal_connection_list(sig["name"]):
...
func _cmd_gridmap(params: Dictionary) -> void:
	...
	match action:
		...
		"get_used":
			...
			var result: Array = []
			for c: Variant in cells.slice(0, 100):
	...
	match action:
		"create":
			...
			if params.has("points"):
				for p: Variant in params["points"]:
		...
		"get_points":
			...
			if node == null or not node is Path3D:
				...
				return
			var pts: Array = []
			for i: Variant in (node as Path3D).curve.point_count:
...
func _get_or_create_environment() -> Environment:
	...
	for child: Variant in get_tree().root.get_children():
...
func _draw():
	for cmd: Variant in draw_commands:
		var p = cmd.params
		var c = cmd.color
		match cmd.action:
			"line":
	...
	match action:
		...
		"get_points":
			...
			if node == null or not node is Path2D:
				...
				return
			var pts: Array = []
			for i: Variant in (node as Path2D).curve.point_count:
...
func _cmd_audio_effect(params: Dictionary) -> void:
	...
	match action:
		"list":
			var effects: Array = []
			for i: Variant in AudioServer.get_bus_effect_count(bus_idx):
...
func _cmd_ui_item_list(params: Dictionary) -> void:
	...
	if node is ItemList:
		...
		match action:
			"get_items":
				var items: Array = []
				for i: Variant in il.item_count:
		...
		match action:
			"get_items":
				var items: Array = []
				for i: Variant in ob.item_count:
...
func _cmd_ui_tabs(params: Dictionary) -> void:
	...
	if node is TabContainer:
		...
		match action:
			"get_tabs":
				var tabs: Array = []
				for i: Variant in tc.get_tab_count():
...
func _cmd_ui_menu(params: Dictionary) -> void:
	...
	match action:
		"get_items":
			var items: Array = []
			for i: Variant in menu.item_count:
...
func _cmd_terrain(params: Dictionary) -> void:
	...
	if action == "create":
		...
		var heights: Array = []
		for i: Variant in range(width * depth):
	...
	match action:
		...
		"modify":
			...
			for z: Variant in range(t_depth):
				for x: Variant in range(t_width):
...
func _terrain_rebuild(mi: MeshInstance3D) -> void:
	...
	for z: Variant in range(depth - 1):
		for x: Variant in range(width - 1):
			...
			for tri: Variant in [[i00, v00], [i10, v10], [i01, v01], [i10, v10], [i11, v11], [i01, v01]]:
