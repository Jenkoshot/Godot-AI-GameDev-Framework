...
1. **Scope identification.** Identify every file the system touches, not just its "main"
   ...
   file(s) under `tests/`, its `project-state/[system]/[system].md`, its rows in `project-state/tweak_guide.md`, and its entry in
...
2. **Mandatory dry-run diff.** Before touching anything, produce a full listing of everything
   ...
   present it to the user for explicit approval — never proceed straight to deletion. This is an
   approval gate scaled up for a harder-to-reverse action — never skip it.
3. **Archive, never destroy.** Once approved:
   - Move the system's scripts/scenes to `project-state/archived_systems/[system]/[timestamp]/`
     ...
     through `git log`.
   - Move `project-state/[system]/[system].md`, and the system's
   ...
   - Remove the system's rows from `project-state/tweak_guide.md` and its entry from
     ...
     when, and where it's archived — in `_overview.md`'s "Organization Notes" section.
...
5. **Documentation consistency, same as any task.** Same as `EXECUTOR.md`'s task-completion step: update
   `_overview.md`, `bugs/master_bugs.md`, the project root `README.md`, and any `design_docs/*.md`
