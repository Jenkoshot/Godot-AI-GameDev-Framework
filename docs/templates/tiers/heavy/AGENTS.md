> [!IMPORTANT]
> THIS FILE IS FOR CHATGPT CODEX ONLY.
> It is NOT for Claude Code or Antigravity (Gemini). If you are not ChatGPT Codex, do not use these instructions.

# AGENTS.md — Micro Execution Engine Rules

This file defines how ChatGPT Codex operates in this game project. It is identical across all
...
ChatGPT Codex is the **executor**, not the planner. Planning, task breakdown, and architecture
decisions belong to Gemini/Antigravity (`GEMINI.md`), which reads the full project state and
writes concrete steps into `project-state/session_state.json`. ChatGPT Codex stays scoped to
...
3. **If either check fails, Tier 1 is unavailable this session — do not stop.** Note the
   ...
   a. **Submodule/build** — `docs/tools/godot-mcp/build/index.js` must exist (relative to the
      `Godot_Projects/` root). If missing: `git submodule update --init --recursive` from
      `Godot_Projects/`, then `cd docs/tools/godot-mcp && npm install && npm run build`.
   b. **Machine path lookup** — read `Godot_Projects/docs/machine_paths.json` (the shared,
      ...
      - **Entry not found** (new machine): scan common install locations for Godot (Steam
        ...
        that fails). Append a new entry to `Godot_Projects/docs/machine_paths.json` keyed by
   ...
   f. **Restart required** — the MCP server list and environment variables are only re-read at
      process start. After any fix above, tell the user to fully restart ChatGPT Codex (not just
...
- Session Log: `./project-state/session_log.md`
- Tweak Guide: `./project-state/tweak_guide.md` (see "Tweak Guide" below)
- Target System State: `./project-state/[system]/[system].md`
- Master Bug Index: `./bugs/_overview.md`
- Target System Bugs: `./bugs/[system]/[system].md`
...
shared `Godot_Projects/` repo — is a short, human-readable, always-current snapshot of what
actually works, so opening the repo answers "what's implemented right now" without reading
`project-state/`. It is derived from `project-state/_overview.md` and `bugs/_overview.md`, not
...
- **Known Issues** — open bugs from `bugs/_overview.md`, one line each.
- **Last Updated** — today's date and the current "Last Verified Commit" hash.
...
`./project-state/session_log.md` is an append-only audit trail — one line per session, appended
by whichever agent ran (ChatGPT Codex here, Gemini/Antigravity in `GEMINI.md`) at the end of every
session, successful or not. It does not replace the per-system status files; it's a cheap way to
make protocol drift visible after the fact instead of assumed — e.g. a system marked
`Verified-in-game` that clearly wasn't tested, or a session that skipped phases.

Format — one line, appended, never edited or reordered:
`YYYY-MM-DD HH:MM | agent=claude|gemini | phase=<phase/step> | target_system=<system or "-"> | outcome=<short outcome>`

Example: `2026-08-03 14:22 | agent=claude | phase=execute | target_system=spiral_layout | outcome=COMPLETED, 2 tests added`
...
A bug's `Status` field moves through four states, tracked in `bugs/[system]/[system].md` and
rolled up in `bugs/_overview.md`: `reported` → `investigating` → `resolved` (linked to the commit
that fixed it) → `verified` (confirmed fixed in-game, not just in the diff). `Severity` is not
...
still carry an open bug at any of these four states.
...
- **A bug** is code that doesn't match what you meant to build. Fix: patch the code. Tracked in
  `bugs/[system]/[system].md` per "Bug Lifecycle" above.
...
If a queued step in `session_state.json` directs reuse of an entry from the shared repositories (Gemini checks for a match before writing steps), copy the necessary `.gd` and `.tscn` files and their companion `.md` docs into the project, then:
...
All shared repositories reside at the root `Godot_Projects/` folder so they are globally accessible to every child game project via the shared documentation system.
...
- `../../mechanic-repository/`: Mechanics Code Repository (scripts, explanations alongside them, master index).
- `../../scene-repository/`: Scene Structure Repository (tscn files, tres resources, explanations alongside them, master index).
- `../../asset-repository/`: Assets Repository (models, audio, textures).

**2. Parent Navigation Rule for Agents**
- **Repository Path Resolution:** Always resolve global repository assets relative to the project root using `../../mechanic-repository/`, `../../scene-repository/`, and `../../asset-repository/`.
- **Fallback Check:** If `../../mechanic-repository/` is not present, check `mechanic-repository/`.

**3. UI Component Policy**
UI systems must always be extracted as dual pairs:
1. The Scene Structure (`.tscn` / `.tres`): Saved under `../../scene-repository/ui/`.
2. The Logic Code (`.gd`): Saved under `../../mechanic-repository/ui/`.
...
1. ALWAYS start by reading `./project-state/session_state.json` to obtain current micro-tasks.
   ...
   for Gemini/Antigravity to populate it.
...
8. If a queued step is ambiguous, contradicts the target system file or blueprint, or requires
   ...
   belongs to Gemini/Antigravity, not ChatGPT Codex.
9. Upon task completion:
   ...
   - Run `/code-review` on the diff before marking anything `Verified-in-game` — catch
     ...
     or advance the "Last Verified Commit" marker on a rejected pass.
   - **If this is a trivial task** (single-file, single-function fix; no new signal/behavior; no
     tri-state status change; no bug opened or closed), skip the five bullets below — just
     append one line to `./project-state/session_log.md` (see "Session Log" above) describing
     what was fixed, and set `status` in `session_state.json` to `"COMPLETED"`. Otherwise (a
     tri-state status actually changed, or a bug opened/closed), do the full ritual:
   - Advance any bug entries this task fixed to `resolved` in `./bugs/[system]/[system].md`,
     linked to this commit's hash (see "Bug Lifecycle" above) — move an entry to `verified` only
     once it's confirmed fixed in-game, not merely diffed, and only clear it from the file at
     that point.
   - Update master status in `./bugs/_overview.md` and `./project-state/_overview.md`.
   - Update this project's root `README.md` — "Working"/"In Progress"/"Known Issues" sections —
     to match the new `_overview.md`/`bugs/_overview.md` state.
   - Advance the "Last Verified Commit" marker: once the headless check (see Verification
     Commands below) passes, run `git rev-parse HEAD` and write that hash into
     `./project-state/_overview.md`'s "Last Verified Commit" field (and into the project root
     `README.md`'s "Last Updated" line).
   ...
   - Append one line to `./project-state/session_log.md` per the format above.
   - **Close Pattern (ADR):** Log any major architectural decisions made during this task (e.g., choosing a specific node type over another) in `project-state/architecture_decisions.md` so future agents don't have to guess the reasoning.
...
`Godot_Projects/` root and wired in via this project's `.mcp.json` (see `SETUP.md` Step 0). Its
...
## Shader Creation Protocol

Whenever you need to create or apply a shader, you must **FIRST** check the other shared repositories (`../../mechanic-repository/`, `../../scene-repository/`, `../../effect-blocks/`, `../../poly-blocks/NatureBlocks/`) to see if an existing shader or `.tres` material already exists and can be creatively adapted. 

If no existing shader fits the requirement and you must write one from scratch, you must navigate to the `../../godot-shader-bible/` folder. Read its AI Directory Guide (`README.md`) and the actual `godot_shader_bible.md` to ensure your new shader aligns with the project's foundational rules, preferred syntax, and snippets.
