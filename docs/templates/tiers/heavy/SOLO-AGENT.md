...
6. Read Current Progress & State:
   ...
   - Master Bugs: `./bugs/_overview.md`
   - Tweak Guide: `./project-state/tweak_guide.md` (see "Tweak Guide" below)
7. Do NOT read individual system markdowns (`./project-state/[system]/[system].md`) yet.
...
2. Read ONLY the relevant system files:
   ...
   - `./bugs/[target_system]/[target_system].md` (that system's bugs only)
...
1. **Effort self-assessment:** Note the complexity of the task you just scoped — low (minor
   ...
   coding.
2. **Write the execution payload (MANDATORY).** Write `./project-state/session_state.json` so the work survives a
...
by whichever agent ran (the Executor Agent here, the Architect Agent in `ARCHITECT.md`) at the end of every
...
`YYYY-MM-DD HH:MM | agent=executor|architect | phase=<phase/step> | target_system=<system or "-"> | outcome=<short outcome>`

Example: `2026-08-03 14:22 | agent=executor | phase=execute | target_system=spiral_layout | outcome=COMPLETED, 2 tests added`
