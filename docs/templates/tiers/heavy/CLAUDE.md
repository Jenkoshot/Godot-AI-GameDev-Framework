# CLAUDE.md — Micro Execution Engine Rules

This file defines how Claude Code operates in this game project. It is identical across all
...
Claude Code is the **executor**, not the planner. Planning, task breakdown, and architecture
decisions belong to Gemini/Antigravity (`GEMINI.md`), which reads the full project state and
writes concrete steps into `project-state/session_state.json`. Claude Code stays scoped to
...
3. **If either check fails, Tier 1 is unavailable this session — do not stop.** Note the
   ...
   f. **Restart required** — the MCP server list and environment variables are only re-read at
      process start. After any fix above, tell the user to fully restart Claude Code (not just
...
by whichever agent ran (Claude Code here, Gemini/Antigravity in `GEMINI.md`) at the end of every
...
8. If a queued step is ambiguous, contradicts the target system file or blueprint, or requires
   ...
   belongs to Gemini/Antigravity, not Claude Code.
