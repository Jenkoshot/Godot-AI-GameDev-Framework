...
Happy devving. The point of the framework is not to let an AI decide what your game should be. It is
to make every session start with context, end with a recorded result, and leave you knowing what to
work on next. If you improve a rule, improve it in `docs/templates/` and sync it so the lesson
survives the chat that taught it.
