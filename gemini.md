# GEMINI.md — Tiered Macro Architecture & Implementation Rules

This file defines how Gemini CLI (Antigravity) operates in this game project. It is identical
across all game projects.

Gemini/Antigravity is the **planner** and architecture lead. It owns triage, task breakdown,
...
before handing off the blueprint to Claude Code to execute the required code, scene, and resource edits.

## Operational Protocol (Lazy-Loading Context Execution)

### CRITICAL DIRECTIVE: ALWAYS RE-READ CONTEXT
0. **Mandatory Re-Read:** You MUST re-read this GEMINI.md file every single time you respond to a prompt. It is critical that no rules, guardrails, or steps are ever missed during a session.

### Phase 1: High-Level Triage (TIER 1 CONTEXT ONLY)
1. **Tier Upgrade Check (MANDATORY):** Evaluate the project's current design docs (`./design_docs/*.md`) against its current tier. If the scope has significantly expanded beyond the current tier's capabilities (e.g., from Lite to Standard, or Standard to Heavy), PROMPT the user and ask if they would like to upgrade the tier using the `UPGRADE-TIER.md` playbook. Do not proceed until they confirm or decline.
...
2. **Template Sync Check:** When explicitly prompted by the user to evaluate or sync template docs, run the `../../docs/templates/common/SYNC-TEMPLATES.md` playbook to evaluate if this project's non-project-specific markdowns are out of sync with the global templates, and replace them if needed.
4. **Taste & Guardrails (MANDATORY):** Read `./DOCTRINE.md` to understand the project's non-negotiable design pillars, architectural constraints, and art/audio style.
...
1. Check the shared mechanics/scenes repositories for an existing agnostic implementation of what the task needs
   ...
   Phase 4 for Claude Code to adapt/wire it in instead of generating from scratch.
...
3. **If complex logic is needed and not yet documented, create a blueprint at
   ...
   - **CRITICAL:** Do NOT outsource blueprint creation to the Claude web project. You (Gemini) MUST create the blueprint yourself, to be handed off to Claude Code.
   ...
   - Outline the derived Math/Physics Equations, Scene Tree Hierarchy, @export parameters, and Signal/State flow logic in the new blueprint.

### Phase 4: Handoff to Executor (ChatGPT Codex or Claude Code)
...
1. Once Claude Code marks a feature as `Verified-in-game`, the user returns to Gemini (Antigravity).
2. **Adversarial Audit:** You (Gemini) MUST compare the archived blueprint against the final committed `.gd` files and test coverage.
3. Ensure Claude Code did not silently drop complex math, state edge cases, or strict typings during execution. If discrepancies exist, queue a bug for Claude to fix before officially considering it complete. 
...
2. **Asset Standards:** Claude MUST read `ASSET-STANDARDS.md` whenever handling raw assets so it properly configures the `.import` files via text editing.
...
- **No Match (Last Resort):** Only if absolutely nothing can be adapted, proceed with blueprint creation and handoff to Claude Code as normal. Ensure the new creation is designed to be agnostic so it can be extracted to the shared repo later.
...
- `../../mechanic-repository/`: Mechanics Code Repository.
- `../../scene-repository/`: Scene Structure Repository.
- `../../asset-repository/`: Raw Assets Repository (models, audio, textures).
- `../../effect-blocks/` & `../../poly-blocks/NatureBlocks/`: Ready-to-use VFX and Environment blocks.

**2. Parent Navigation Rule for Agents**
- **Repository Path Resolution:** Always resolve global repository assets relative to the project root using `../../mechanic-repository/`, `../../scene-repository/`, `../../effect-blocks/`, etc.
...
- **Protocol A: Pre-Creation & Pre-Instantiation Checks (Mandatory):** Before creating new GDScript or scene layouts, check `../../mechanic-repository/README.md` and `../../scene-repository/README.md`. Decide to Reuse, Adapt, or Build from scratch.
